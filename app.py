...
with col_right:
    ...
    if not evaluar:
        ...
        try:
            ...
            # Creamos el explainer de SHAP en vivo para evitar caídas en Linux
            import shap
            explainer = shap.TreeExplainer(modelo)

            # Preprocesamiento de los datos del candidato
            candidato_t = preprocessor.transform(candidato_df[ALL_FEATURES])
            candidato_proc = pd.DataFrame(candidato_t, columns=feature_names)

            # ------------------------------------------------------------------
            # ¡ESTAS SON LAS LÍNEAS QUE FALTABAN!
            # Calculamos la probabilidad y la predicción final
            # ------------------------------------------------------------------
